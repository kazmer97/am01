...
ggplot(top25_wine, aes(y = reorder(country, spirit_servings), x = spirit_servings ))+
  geom_col()+
  NULL
...
ratingByGenre <- movies%>%
  group_by(genre)%>%
  summarise(rating, mean_rating = mean(rating),
            max_rating = max(rating),
            min_rating = min(rating),
            sd_rating = sd(rating),
            median_rating = median(rating))

ggplot(ratingByGenre, aes(y = rating, x = genre))+
  geom_boxplot()
...
ggplot(monthlyReturnSummary, aes(x = monthly_sd, y = monthly_mean))+
  ...
  ggrepel::geom_text_repel(aes(label = symbol), size = 10)
