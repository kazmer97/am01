...
ggplot(approval_polllist_net, aes(x=week, y=mean_net_approve))+
  geom_point() +
  geom_smooth()+
  geom_line()+
  geom_line(aes(x = week, y = lower))+
  geom_line(aes(x = week, y = upper))+
  theme_bw()
```
...
# world_hiv <- hiv_longer%>%
#   left_join(region,by =c("country"))%>%
#   left_join(worldbank_data, by=c("date","country"))%>%
...
worldbank_join %>% 
  ...
  # slice_max(n = 5, order_by = SH.DYN.MORT)
...
```{r}
bike_2016_2019_month_avg <- bike%>%
  filter(day > dmy("01/01/2016"), day<dmy("01/01/2020"))%>%
  group_by(month)%>%
  summarise(monthly_avg = mean(bikes_hired),
         daily_avg = mean(bikes_hired))


bike_graph <- bike%>%
  filter(day > dmy("01/01/2016"))%>%
  
  # geom_line(bike_2016_2019_month_avg, aes(x = month, y = monthly_avg))+
